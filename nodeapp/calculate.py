...
PARAMETERS = {
    "length": 60,                      # length of the field
    "width": 60,                       # width of the field
...
class Simulation:
    def __init__(self, parameters):
        ...
        self.df = pd.DataFrame(columns=["Yield", "Growth", "Water", "Overlap"])
    ...
    def record_data(self, size, growth_rate, water_level, overlap):
        new_row = pd.DataFrame([[size, growth_rate, water_level, overlap]], columns=self.df.columns)
...
def main():
    ...
    sim.planting()
    sim.run_simulation()
    packed_array = msgpack.packb(sim.size_layer.tolist())
    packed_df = msgpack.packb(sim.df.to_dict(orient='records'))
    sys.stdout.buffer.write(packed_array)
    sys.stdout.buffer.write(packed_df)
    sys.stdout.flush()
    print(sim.df.to_dict(orient='records'))
    print(sim.size_layer.tolist())
