            ...
            for row in self.plants:
                for plant_in_row in row:
                    if  plant_in_row.plant in Crop_list:
                        debug_print(f"Plant at x:{plant_in_row.x_coordinate}, y:{plant_in_row.y_coordinate}, crop: {plant_in_row.plant.plant['name']}, with BBCH: {round(plant_in_row.bbch,2)}, Waterlevel: {waterlevel}, Soiltype: {soiltype.name}")     
                        soil_water = plant_in_row.grow(self.crop, weatherdata, waterlevel, soiltype)
                        self.waterlevel = soil_water
                    elif plant_in_row.plant['name'] in [weed.plant['name'] for weed in weed_list]:
                        debug_print(f'Weed{plant_in_row.plant["name"]} at x:{plant_in_row.x_coordinate}, y:{plant_in_row.y_coordinate}, with BBCH: {round(plant_in_row.bbch)}, Waterlevel: {waterlevel}, Soiltype: {soiltype.name}')
                        plant_in_row=plant_in_row.grow(self.crop, weatherdata, waterlevel,soiltype)
                    elif plant_in_row.plant['name'] == 'Empty':
        ...
        def plot_plants_on_field(self):
            ...
            for i in range(len(self.plants)):
                for j in range(len(self.plants[0])):
                    if self.plants[i][j].plant in Crop_list:
                        data[i][j] = 1  # Set 1 for crop
                    elif self.plants[i][j].plant['name'] == 'Field Vetch':
                        data[i][j] = 2  # Set 2 for field vetch
...
#        wed_factor=self.check_weed_impact(Farm.Field.plants)   
        ...
        #self.check_harvest()
        self.bbch += 1*wtr_factor*tmp_factor#*wed_factor
        return soil_water
    ...
    def weed_germination(self, timestring):
        ...
        for weed in weed_list:
            if True:  # weed.plant.get('earliest_appearance') <= date <= weed.plant.get('latest_appearance'):
                if random.random() < 0.1 and random.random() < 0.1 :  # '*tmp_factor'
    ...
    def check_overgrwth(self):
        bbch = self.bbch
        stage1 = self.plant.plant["stage1"]
        stage2 = self.plant.plant["stage2"]
        stage3 = self.plant.plant["stage3"]
        stage4 = self.plant.plant["stage4"]
        
        if bbch <= stage1["BBCH"]:
            fct = bbch / stage1["BBCH"]
            diameter = fct * stage1["diameter"]
        elif bbch <= stage2["BBCH"]:
            fct = (bbch - stage1["BBCH"]) / (stage2["BBCH"] - stage1["BBCH"])
            diameter = stage1["diameter"] + fct * (stage2["diameter"] - stage1["diameter"])
        elif bbch <= stage3["BBCH"]:
            fct = (bbch - stage2["BBCH"]) / (stage3["BBCH"] - stage2["BBCH"])
            diameter = stage2["diameter"] + fct * (stage3["diameter"] - stage2["diameter"])
        elif bbch <= stage4["BBCH"]:
            fct = (bbch - stage3["BBCH"]) / (stage4["BBCH"] - stage3["BBCH"])
            diameter = stage3["diameter"] + fct * (stage4["diameter"] - stage3["diameter"])
        else:
            # If bbch is greater than stage4["BBCH"], the diameter raises with the same rate as stage4["diameter"]
            diameter = stage4["diameter"] + (bbch - stage4["BBCH"])
        diameter = round(diameter, 1)

    ### Function to check if the plant is surrounded by weeds
    def check_weed_impact(self,plant_list):                
        ...
        for i in range(xcoord - 1, xcoord + 2):
            for j in range(ycoord - 1, ycoord + 2):
                ...
                if self.plant_list[i][j] in weed_list:
                    weedfactor += 1
        if weedfactor == 8:
            factor = 0
        elif weedfactor == 0:
...
def check_waterlevel(crop,weatherdata,waterlevel,soiltype,soil_thickness=30):
    ...
    optimal_water=crop.plant["optimal_water"]
...
csv_file = "/home/student/NR_Simulation/NRSimulation/Simulation/transformed_weather_data.csv"
...
weed_list = [Field_vetch,Jacobsragwort]# Jacobsragwort, Dog_Chamomile, Shepherds_Purse, Goosefoot, Bird_Knotweed, Report]
